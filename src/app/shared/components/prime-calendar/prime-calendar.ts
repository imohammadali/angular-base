    ...
    >
      <ng-template #inputContainer [ngIf]="!inline">
        <input
          ...
          [ngClass]="'p-inputtext p-component p-inputtext-sm width-100'"
          autocomplete="off"
        ...
        <button
          ...
          pRipple
          *ngIf="showIcon"
          (click)="jumpDayOverlay.toggle($event)"
        ...
        <p-overlayPanel #jumpDayOverlay [style]="{'margin-top': 'unset'}" [dismissable]="true">
          <div>
            <button (click)="jumpDaySubmit(0);" style="width: -webkit-fill-available;width: -moz-available;width: fill-available" class='btn btn-outline-secondary' translate="feedback.today"></button>
          </div>
          <div>
            <span>
              <button (click)="jumpDaySubmit(-1);" class='btn btn-outline-secondary' translate="feedback.previous_day"></button>
            </span>
            <span>
              <button (click)="jumpDaySubmit(1);" class='btn btn-outline-secondary' translate="feedback.next_day"></button>
            </span>
          </div>
          <div>
            <span>
              <button (click)="jumpDaySubmit(-7);" class='btn btn-outline-secondary' translate="feedback.previous_week"></button>
            </span>
            <span>
              <button (click)="jumpDaySubmit(7);" class='btn btn-outline-secondary' translate="feedback.next_week"></button>
            </span>
          </div>
          <div>
            <span>
              <button (click)="jumpDaySubmit(-30);" class='btn btn-outline-secondary' translate="feedback.previous_month"></button>
            </span>
            <span>
              <button (click)="jumpDaySubmit(30);" class='btn btn-outline-secondary' translate="feedback.next_month"></button>
            </span>
          </div>
        </p-overlayPanel>
      </ng-template>
            ...
            >
              <div class="p-datepicker-header">
                ...
                >
                  ...
                  >
                    <ng-template
                      *ngTemplateOutlet="previousIconTemplate"
                    ></ng-template>
                  </span>
                </button>
                ...
                >
                  ...
                  <span *ngIf="nextIconTemplate" class="p-datepicker-next-icon">
                    <ng-template
                      *ngTemplateOutlet="nextIconTemplate"
                    ></ng-template>
                  </span>
                </button>
              </div>
        ...
        >
          <div class="p-minute-picker">
            ...
            >
              ...
              <ng-template
                *ngTemplateOutlet="incrementIconTemplate"
              ></ng-template>
            </button>
            <span
            ...
            >
              ...
              <ng-template
                *ngTemplateOutlet="decrementIconTemplate"
              ></ng-template>
            </button>
          </div>
...
export class PrimeCalendar implements OnInit, OnDestroy, ControlValueAccessor {
  ...
  @ViewChild('jumpDayOverlay', { static: false }) jumpDayOverlay: OverlayPanel;
      ...
      if (!this.disabled) {
        ...
        if (this.numberOfMonths === 1) {
          this.contentViewChild.nativeElement.style.width =
            DomHandler.getOuterWidth(this.containerViewChild.nativeElement) +
            'px';
        }
      }
    }
  ...
  getWeekNumber(date: moment.Moment) {
    ...
    checkDate.jDate(checkDate.date() + 4 - (checkDate.jDay() || 7));
  ...
  createMonth(month: number, year: number) {
    ...
    for (let i = 0; i < monthRows; i++) {
      ...
      if (this.showWeek) {
        weekNumbers.push(
          this.getWeekNumber(moment([week[0].year, week[0].month, week[0].day]))
        );
      }
  ...
  onUserInput(event: any) {
    ...
    if (this.isSingleSelection()) {
      if (this._showTime) {
        let parts = val.split(' ')
        val = moment.from(parts?.[0], 'fa').format(this.getDateFormatForMoment())
        val = `${val} ${parts?.[1]}`
      } else {
        val = moment.from(val, 'fa').format(this.getDateFormatForMoment())
      }
    }
  ...
  isValidSelection(value: any): boolean {
    ...
    if (this.isSingleSelection()) {
      if (
        !this.isSelectable(value.date(), value.jMonth(), value.jYear(), false)
      ) {
        isValid = false;
      }
    } else if (
      value.every((v: any) =>
        this.isSelectable(v.date(), v.jMonth(), v.jYear(), false)
      )
    ) {
      if (this.isRangeSelection()) {
  ...
  updateUI() {
    ...
    let val =
      this.defaultDate && this.isValidDate(this.defaultDate) && !this.value
        ...
        ? propValue
        : moment();
    val = typeof val === 'string' ? moment(val) : val;
      ...
      if (this.appendTo) {
        if (this.view === 'date') {
          ...
          this.overlay.style.minWidth =
            DomHandler.getOuterWidth(this.inputfieldViewChild.nativeElement) +
            'px';
        } else {
          this.overlay.style.width =
            DomHandler.getOuterWidth(this.inputfieldViewChild.nativeElement) +
  ...
  getDateFormatForMoment() {
    let temp = [];
    this.getDateFormat().split('').forEach((y, i) => {
      switch (y) {
        case 'y': {
          temp.push('Y', 'Y')
          break
  ...
  formatDate(date: any, format: any) {
    ...
    date = typeof date === 'string' ? moment(date).locale('fa') : date;
    ...
    let iFormat: number,
      ...
      shortYearCutoff =
        typeof this.shortYearCutoff !== 'string'
          ...
          : (moment().jYear() % 100) + parseInt(this.shortYearCutoff, 10),
      year = -1,
      month = -1,
      day = -1,
      doy = -1,
  ...
  jumpDaySubmit(count) {
    if (count > 0) {
      ...
      this.updateModel(this.value.add(count, 'day'))
    } else if (count < 0) {
      if (!this.value) this.value = moment()
      this.updateModel(this.value.subtract(Math.abs(count), 'day'))
    } else {
      this.updateModel(moment())
    }
